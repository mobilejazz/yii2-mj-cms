...
EOD;
    ...
    public function actionExtract($configFile = null)
    {
        ...
        $config                  = array_merge($this->getOptionValues($this->action->id), $configFileContent, $this->getPassedOptionValues());
        $config[ 'messagePath' ] = Yii::getAlias($config[ 'messagePath' ]);

        if (!isset($config[ 'languages' ]))
        {
            throw new Exception('The configuration file must specify "languages".');
        }
        ...
        foreach ($config[ 'sourceFolders' ] as $subFolder)
        {
            $folder = Yii::getAlias($subFolder);
            $sourceFolder = realpath($folder);
            $files        = array_merge($files, FileHelper::findFiles($sourceFolder, $config));
        }

        $messages = [ ];
        foreach ($files as $file)
        {
            $messages = array_merge_recursive($messages, $this->extractMessages($file, $config[ 'translator' ], $config[ 'ignoreCategories' ], $config[ 'categoryOverrides' ]));
        }
     ...
     * @return array
     */
    protected function extractMessages($fileName, $translator, $ignoreCategories = [ ], $categoryOverrides = [])
    {
        ...
        foreach($categoryOverrides as $pattern => $override){

            if(preg_match($pattern, $fileName) > 0){
                $this->stdout("Category override match. Pattern = $pattern, fileName = $fileName, override = $override");
        ...
        foreach ((array) $translator as $currentTranslator)
        {
            ...
            $messages = array_merge_recursive($messages, $this->extractMessagesFromTokens($tokens, $translatorTokens, $ignoreCategories, $categoryOverride));
        }
     ...
     */
    private function extractMessagesFromTokens(array $tokens, array $translatorTokens, array $ignoreCategories, $categoryOverride = null)
    {
        $messages                = [ ];
                ...
                if ($this->tokensEqual(')', $token))
                {
                    ...
                    if ($pendingParenthesisCount === 0)
                    {
                        ...
                        if (isset($buffer[ 0 ][ 0 ], $buffer[ 1 ], $buffer[ 2 ][ 0 ]) && $buffer[ 0 ][ 0 ] === T_CONSTANT_ENCAPSED_STRING && $buffer[ 1 ] === ',' && $buffer[ 2 ][ 0 ] === T_CONSTANT_ENCAPSED_STRING)
                        {
                            ...
                            if($categoryOverride) {
                                $this->stdout("Overriding category from $category to $categoryOverride\n");
     ...
     * - `myapp*` - will be ignored by all categories beginning with `myapp` (`myapp`, `myapplication`, `myapprove`, `myapp/widgets`,
     * `myapp.widgets`, etc).
     *
    ...
    protected function saveMessagesCategoryToPHP($messages, $fileName, $overwrite, $removeUnused, $sort, $category, $markUnused)
    {
        if (is_file($fileName))
        {
            ...
            foreach ($existingMessages as $message => $translation)
            {
                ...
                {
                    if (!empty($translation) && (!$markUnused || (strncmp($translation, '@@', 2) === 0 && substr_compare($translation, '@@', -2,
                                    2) === 0))
                    )
                    {
...
EOD;
    ...
    protected function saveMessagesToDb($messages, $db, $sourceMessageTable, $messageTable, $removeUnused, $languages, $markUnused)
    {
        ...
        else
        {
            ...
            {
                $db->createCommand()
                   ->update($sourceMessageTable, [ 'message' => new \yii\db\Expression("CONCAT('@@',message,'@@')") ], [ 'in', 'id', $obsolete ])
                   ->execute();
                $this->stdout("updated.\n");
            }
            else
